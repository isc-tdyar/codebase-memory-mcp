...
void cbm_macro_table_init_system(CBMMacroTable *t) {
    ...
    for (int i = 0; SYSTEM_MACROS[i].name; i++) {
        if (t->count >= CBM_MACRO_TABLE_CAP) break;
        ...
        for (int p = 0; p < CBM_MACRO_MAX_PARAMS; p++) e->param_names[p] = NULL;
    }
}

void cbm_macro_table_add(CBMMacroTable *t, CBMArena *arena,
                         const char *name, int param_count,
                         const char **param_names,
                         const char *expansion, const char *resolved_callee) {
    if (t->count >= CBM_MACRO_TABLE_CAP || !name) return;
    for (int i = 0; i < t->count; i++) {
        if (strcasecmp(t->entries[i].name, name) == 0) return;
    }
    CBMMacroEntry *e = &t->entries[t->count++];
    ...
    for (int p = 0; p < CBM_MACRO_MAX_PARAMS; p++) {
        e->param_names[p] = (param_names && p < param_count) ? cbm_arena_strdup(arena, param_names[p]) : NULL;
    }
}
...
const CBMMacroEntry *cbm_macro_table_find(const CBMMacroTable *t, const char *name) {
    ...
    for (int i = 0; i < t->count; i++) {
        if (strcasecmp(t->entries[i].name, name) == 0) return &t->entries[i];
    }
    return NULL;
}
...
void cbm_parse_inc_file(CBMMacroTable *t, CBMArena *arena, const char *content) {
    ...
    while (*line) {
        ...
        if (strncmp(p, "#define", 7) == 0 && (p[7] == ' ' || p[7] == '\t')) {
            ...
            while (*p && *p != '(' && *p != ' ' && *p != '\t' && p < end) p++;
            if (p == name_start) goto next_line;

            char name[256];
            int nlen = (int)(p - name_start);
            if (nlen >= (int)sizeof(name)) goto next_line;
            ...
            if (*p == '(') {
                ...
                while (*p && *p != ')' && p < end) {
                    ...
                    while (plen > 0 && (pn_start[plen-1] == ' ' || pn_start[plen-1] == '\t')) plen--;
            ...
            while (explen > 0 && (p[explen-1] == '\r' || p[explen-1] == ' ' || p[explen-1] == '\t')) explen--;
            char *expansion = NULL;
            if (explen > 0) {
            ...
            cbm_macro_table_add(t, arena, name, param_count,
                                param_count > 0 ? param_name_ptrs : NULL,
                                expansion, NULL);
        }
...
char *cbm_macro_expand(CBMArena *arena, const CBMMacroEntry *entry,
    ...
    while (*p && out < (int)sizeof(buf) - 1) {
        if (*p == '%') {
            ...
            for (int i = 0; i < entry->param_count && i < CBM_MACRO_MAX_PARAMS; i++) {
                if (!entry->param_names[i]) continue;
...
char *cbm_macro_extract_callee(CBMArena *arena, const char *expansion) {
    ...
    if (p && p[2] != '$') {
        ...
        if (*tag_end == '^') {
            ...
            while (*rtn_end && *rtn_end != '(' && *rtn_end != ' ') rtn_end++;
