...
- [X] T001 Add `CBM_LANG_OBJECTSCRIPT_EXPORT` to `CBMLanguage` enum in `cbm.h`
- [X] T002 Create `internal/cbm/iris_export_xml.h` with public API:
  `cbm_iris_export_to_udl(CBMArena*, const char* xml, int len, int* class_count) → char**`
- [X] T003 Add `iris_export_xml.c` to `Makefile.cbm` EXTRACTION_SRCS

## Phase 2: Tests (TDD — write BEFORE implementation)

- [X] T004 [P] Test `iris_export_xml_simple_class`: minimal Export with one method,
  assert UDL contains class name, method name, method body, Extends clause
- [X] T005 [P] Test `iris_export_xml_classmethod`: `<ClassMethod>1</ClassMethod>` →
  UDL emits `ClassMethod` keyword, `FormalSpec` → correct args
- [X] T006 [P] Test `iris_export_xml_property_parameter_index`: Property with Type+MAXLEN,
  Parameter with Default, Index with Unique → correct UDL syntax for each
- [X] T007 [P] Test `iris_export_xml_calls_extracted`: full extract of XML fixture
  where method body calls `##class(X).Method()` → CALLS edge to X.Method exists
- [X] T008 [P] Test `iris_export_xml_multi_class`: Export with two `<Class>` blocks →
  `class_count == 2`, both class names present in respective UDL strings
- [X] T009 Register T004-T008 in test runner

**Phase gate**: all 5 tests MUST FAIL before proceeding.

## Phase 3: Transcoder implementation

- [X] T010 Implement `iris_export_xml.c`:
  - CDATA scanner (`<![CDATA[` ... `]]>`)
...
- [X] T011 In `discover.c`: change Export file classification from `CBM_LANG_COUNT`
  (skip) to `CBM_LANG_OBJECTSCRIPT_EXPORT`
- [X] T012 In `pass_definitions.c`, `pass_calls.c`, `pass_parallel.c`: for
...
- [X] T013 Re-index hscommunity, confirm:
